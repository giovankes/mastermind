...
for i in range(play_number):  # start playing each game
    ...
    while A < 4:  # Still cleaning the code_set until we find the real answer
        ...
        guess = chooseone(
            code_set
        )  # We use the chooseone function to pick the element in the code set for next play
        A, B = playresult(code, guess)
        # Store the data to csv
    with open("mastermind_final.csv", "a") as f:
...
print(
    "Average number of count: ", total_num / play_number
